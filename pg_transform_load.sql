...
--
-- Create the running entries table
--

\echo Drop and Create table "running_entries"

-- There is no "IF EXISTS" in older versions of postgres
DROP TABLE running_entries;

CREATE TABLE  running_entries (
       id INTEGER PRIMARY KEY, 
       url TEXT, entry_date TIMESTAMP, 
       title TEXT, felt TEXT, elapsed_time INTERVAL NOT NULL DEFAULT '59:59:59'::interval, 
...
--
\echo Inserting running entries...
INSERT INTO running_entries (id, url, entry_date, title, felt, description, effort_outof_5, gear, weather, calories)
SELECT id, url, entry_date, title, felt, description, effort_outof_5, gear, weather, calories from imported_entries
WHERE activity_type = 'Running';

\echo Updating distance_in_km
UPDATE running_entries SET distance_in_km = miles_to_km (i.distance) FROM imported_entries i where i.distance_units = 'miles' AND running_entries.id = i.id;
UPDATE running_entries SET distance_in_km = i.distance FROM imported_entries i where i.distance_units = 'kilometers' AND running_entries.id = i.id;

\echo Updating distance_in_miles
UPDATE running_entries SET distance_in_miles = km_to_miles (i.distance) FROM imported_entries i where i.distance_units = 'kilometers' AND running_entries.id = i.id;
UPDATE running_entries SET distance_in_miles = i.distance FROM imported_entries i where i.distance_units = 'miles' AND running_entries.id = i.id;


\echo updating duration_time_seconds
UPDATE running_entries SET elapsed_time = COALESCE(i.duration_seconds * INTERVAL '1 second', '0 seconds'::INTERVAL) FROM imported_entries i WHERE running_entries.id = i.id;

\echo Setting pace_per_mile
UPDATE running_entries SET pace_per_mile = elapsed_time / distance_in_miles where distance_in_miles != 0;
UPDATE running_entries SET pace_per_mile = '59:59:59' where pace_per_mile = '00:00:00';

\echo Setting pace_per_km
UPDATE running_entries SET pace_per_km = elapsed_time / distance_in_km where distance_in_km != 0;
UPDATE running_entries SET pace_per_km = '59:59:59' where pace_per_km = '00:00:00';


-- Sample:
-- fitness=# select id,title, elapsed_time, distance_in_miles, distance_in_km, pace_per_mile, pace_per_km 
-- from running_entries WHERE pace_per_mile between '5 minutes'::interval AND '7 minutes'::interval order by pace_per_mile limit 20; 
...
0--  24524049 | Time trial 1600m and 400m              | 00:06:45     |            1.2500 |         2.0000 | 00:05:24        | 00:03:22.5
